...
const QuestionBank = () => {
  ...
  useEffect(() => {
    dispatch(setPageTitle("Course Artifacts — Coordinator View"));
  }, [dispatch]);
  ...
  const referenceItems: ReferenceItem[] = [
    ...
    },
    {
      ...
      title: "Pedagogy",
      subtitle:
        dynamicPedagogyUnits.length > 0 ? "Teaching Approaches" : "Curriculum Approaches",
    ...
    },
    {
      ...
      title: "Lesson Plan",
      subtitle:
        dynamicLessonUnits.length > 0 ? "Course Delivery Plan" : "Delivery Schedule",
    ...
    <div className="min-h-screen bg-gray-50 dark:bg-gray-900 pb-8">
      <CourseBanner
        ...
        onCourseChange={(val) => {
          ...
          if (selected) {
            router.push(
              `/neurobe/course-artifacts?course_id=${selected.id}&code=${selected.course_code || selected.code}`
            );
          }
        }}
        toogle="coordinator"
        ...
        onViewChange={(view) => {
          ...
          if (view === "instructor") {
            router.push(
              `/neurobe/ins-course-artifacts?course_id=${state.courseData?.id || router?.query?.course_id || ""}&code=${activeCourseCode}`
            );
          }
        }}
      />

      {/* ── Coordinator Action Bar with Switch to Instructor View ── */}
      <div className="mx-6 mt-4 mb-3 flex flex-wrap items-center justify-between gap-3 rounded-2xl border border-purple-200 bg-gradient-to-r from-purple-50 via-indigo-50 to-purple-50 p-4 shadow-xs dark:border-purple-800/60 dark:from-purple-950/40 dark:via-indigo-950/30 dark:to-purple-950/40">
        <div className="flex items-center gap-3">
          <div className="flex h-10 w-10 shrink-0 items-center justify-center rounded-xl bg-purple-600 text-white shadow">
          ...
          <div>
            <div className="flex items-center gap-2">
              ...
              <span className="rounded-full border border-purple-300 bg-purple-100 px-2.5 py-0.5 text-[10px] font-bold text-purple-700 dark:bg-purple-900/40 dark:text-purple-300">
                Full Curriculum Master
              </span>
            </div>
            <p className="text-xs text-slate-600 dark:text-slate-400 mt-0.5">
              Review and manage the active academic artifacts and approved curriculum references for this course.
            </p>
          </div>
        </div>
        <button
          ...
          className="flex shrink-0 items-center gap-1.5 rounded-xl border border-purple-300 bg-white px-4 py-2 text-xs font-bold text-purple-700 shadow-sm transition-all hover:bg-purple-50 active:scale-95 dark:border-purple-700 dark:bg-slate-800 dark:text-purple-300 dark:hover:bg-slate-700"
        >
      ...
      <PageHeader
        ...
        record2="Course Coordinator View"
        record3="Full Master"
      />
      ...
      <div className="mt-5 grid grid-cols-1 lg:grid-cols-12 gap-6 items-start">
        ...
        <div className="lg:col-span-4 xl:col-span-3">
          <CourseReferencesCard
            ...
            availableCountText={`${referenceItems.length + assessmentItems.length} Available References`}
