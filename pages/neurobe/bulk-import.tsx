  ...
  const importFile = async () => {
    try {
      if (state.selectedFile) {
        ...
        if (state.importType === "user") {
          const response: any = await Models.user_import.import(state.selectedFile, orgId);
          ...
          const response: any = await Models.course_import.import(state.selectedFile, orgId);
            ...
            <div className="panel flex flex-col justify-between px-6 py-6">
              ...
              {state.importType === "user" && (
                <div className="mt-4 border-t border-gray-100 pt-3 dark:border-gray-700">
                  <div className="flex items-center gap-2 rounded-lg bg-blue-50/60 dark:bg-blue-900/20 px-3.5 py-2.5 text-xs text-blue-700 dark:text-blue-300">
                    <CheckCircle className="h-4 w-4 shrink-0 text-blue-500" />
                    <span>Passwords are auto-generated and emailed to newly imported users upon completion.</span>
                  </div>
                </div>
              )}
            </div>
